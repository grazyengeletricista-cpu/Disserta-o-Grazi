...
Neste Capítulo, foi discutida a fundamentação teórica dos principais conceitos relacionados à análise de desempenho e à eficiência energética de usinas fotovoltaicas. Na Seção 3.1 é abordada sobre a utilização de microinversores na geração distribuida; na Seção 3.2 é apresentada o conceito de carport solar; na Seção 3.3 são discutidos sobre os principais índices de desempenho; na Seção 3.4 é mencionado sobre operação e manutenção e na Seção 3.5 é abordado sobre o foco do trabalho no Brasil e em Sergipe. ALTERAR TEXTO
\section{GERAÇÃO DISTRIBUIDA}

Na geração distribuída, os microinversores são comumente utilizados para realizar o monitoramento da geração de energia em sistemas de pequeno e médio porte. Estes equipamentos têm ganhado espaço por permitir o rastreamento individualizado do Ponto de Máxima Potência (MPPT) por módulo, mitigando as perdas por sombreamento parcial e por incompatibilidade (PARENTE, 2021), que são fatores críticos para a eficiência neste tipo de sistema.

\section{CARPORT SOLAR}

O carport solar é uma estrutura metálica, que serve como cobertura para áreas de estacionamento, utilizando os painéis fotovoltaicos instalados em seu telhado inclinado. As estruturas de carport aproveitam espaços já construídos, como estacionamentos, para integrar a geração fotovoltaica, oferecendo sombreamento e, frequentemente, servindo como ponto de recarga para veículos elétricos (KULIK, 2024).
\section{ÍNDICES DE DESEMPENHO}

Para avaliar a viabilidade e o desempenho dessas instalações, são utilizados indicadores chave de desempenho (KPIs), fundamentais para identificação de falhas (REDISKE et al., 2024). Dentre os índices  de performace mais relevantes, destacam-se a Produtividade Final (Yf), o Fator de Capacidade (CUF) e a Taxa de Desempenho (PR). Estes índices permitem diagnosticar a qualidade da conversão da energia solar em energia elétrica, identificar perdas operacionais, e comparar o desempenho de sistemas em diferentes localidades e com distintas tecnologias.
Os indicadores chave de desempenho (KPIs) aplicados deste trabalho estão descritos a seguir:
\newpage

\textbf{a. Taxa de  Desempenho (PR)}


A taxa de desempenho é um importante indicador para apontar a eficiência geral do sistema fotovoltaico em relação a incidência de irradiação (PARENTE, 2021). Esse indicador consiste na divisão entre a produtividade final (Yf) e o rendimento de referência (Yr) que é dado pela divisão entre a irradiância no plano do módulo em kW/m² (Ht) e a irradiância de referência (G) no valor de 1kW/m² (REDISKE et al., 2024). Assim sua equação é dada por:
...
Os sete indicadores chave de desempenho (KPIs) apresentados são de extrema importância para comparar e analisar o desempenho dos 53 microinversores presentes na usina a fim de detectar falhas ou mau funcionamento em alguma operação.
...
\section{ADICIONAR O QUE VEM SENDO FEITO NO BRASIL E SERGIPE RELACIONADO AO FOCO DO TRABALHO}
...
Neste Capítulo é apresentado a metodologia do desenvolvimento de um estudo de caso para análise de desempenho e eficiência energética da usina fotovoltaica em modelo carport solar instalada no Sergipe Parque Tecnológico (SergipeTec), com sede em São Cristóvão/SE. Com o intuito de certificar a confiabilidade do sistema e auxiliar na detecção de possíveis falhas, facilitando a tomada de decisões. Além do cumprimento dos objetivos propostos. 
...
O diagrama de bloco mostra de forma simplificada toda a estrutura e os passos dessa dissertação, ilustrada na Figura 4.3: Diagrama de Blocos.
